...
describe('TestRegistrar', () => {
  it('registers names', async () => {
    ...
    await testRegistrar.write.register([labelhash('beam'), accounts[0].address])

    await expect(ensRegistry.read.owner([zeroHash])).resolves.toEqualAddress(
      testRegistrar.address,
    )
    await expect(
      ensRegistry.read.owner([namehash('beam')]),
  ...
  it('forbids transferring names within the test period', async () => {
    ...
    await testRegistrar.write.register([labelhash('beam'), accounts[1].address])

    await expect(testRegistrar)
      .write('register', [labelhash('beam'), accounts[0].address])
