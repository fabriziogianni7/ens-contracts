...
const targetNode = namehash('beam')
  ...
  describe('dns', () => {
    describe('records', () => {
      it('permits setting name by owner', async () => {
        ...
        await expect(
          publicResolver.read.dnsRecord([
            targetNode,
            keccak256(dnsEncodeName('a.beam.')),
            1,
          ]),
        ...
        await expect(
          publicResolver.read.dnsRecord([
            targetNode,
            keccak256(dnsEncodeName('b.beam.')),
            1,
          ]),
      ...
      it('should update existing records', async () => {
        ...
        // a.beam. 3600 IN A 4.5.6.7
        const arec = '016103657468000001000100000e10000404050607' as const
        // beam. 86400 IN SOA ns1.ethdns.xyz. hostmaster.test.beam. 2018061502 15620 1800 1814400 14400
        const soarec =
      ...
      it('should keep track of entries', async () => {
        ...
        await expect(
          publicResolver.read.hasDNSRecords([
            targetNode,
            keccak256(dnsEncodeName('c.beam.')),
          ]),
        ).resolves.toEqual(true)
        await expect(
          publicResolver.read.hasDNSRecords([
            targetNode,
            keccak256(dnsEncodeName('d.beam.')),
          ]),
        ).resolves.toEqual(false)
      ...
      it('should handle single-record updates', async () => {
        ...
        await expect(
          publicResolver.read.dnsRecord([
            targetNode,
            keccak256(dnsEncodeName('e.beam.')),
            1,
          ]),
